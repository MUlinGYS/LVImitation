<template>
	<div style="
		...
		">
		<div class="guide-container-img" :style="{
		...
		<div class="guide-container-menu">
			<div v-for="(item, index) in menu" :key="index" :class="{ active: boldStates[index] }"
				style="padding: 0 10px" @click="
					toggleBold(index);
				scrollToFirst(index);
				">
				{{ item.name }}
		...
		<div class="productsList">
			<div ref="productsDiv" style="
				...
				">
				<div class="product-div" v-for="(product, itemIndex) in products || []" :key="itemIndex" :style="{
					width: boxWidth + 'px',
					height: boxWidth + 120 + 'px',
					marginTop: '5px',
					marginBottom: '5px',
				}" @click="GuideId(product.id)">
					<div class="productsImg" :style="{
						width: boxWidth + 'px',
						height: boxWidth - 20 + 'px',
					}">
						<img :src="`${product.img}`" style="background-size: contain; width: 90%" />
					</div>
					<div class="Price">
